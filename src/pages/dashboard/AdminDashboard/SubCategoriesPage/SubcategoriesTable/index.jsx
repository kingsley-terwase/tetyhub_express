...
export default function SubcategoriesTable({ subcategories, loading, categoryMap, fg, border, main, onView, onEdit }) {
    if (loading) {
        return <Typography sx={{ fontSize: 13, color: fg.tertiary, px: 2.4, py: 3, textAlign: "center" }}>Loading subcategories...</Typography>;
    }

    if (subcategories.length === 0) {
        return <Typography sx={{ fontSize: 13, color: fg.tertiary, px: 2.4, py: 3, textAlign: "center" }}>No subcategories yet.</Typography>;
    }
            ...
            {subcategories.map((sub, i) => (
                ...
                >
                    ...
                    <Box sx={{ flex: 2, minWidth: 0 }}>
                        <Typography sx={{ fontSize: 13.5, fontWeight: 700, color: fg.primary }}>{sub.name}</Typography>
                        <Typography sx={{ fontSize: 11, color: fg.tertiary }}>
                            {categoryMap[sub.category_id] || `Category #${sub.category_id}`}
                        </Typography>
                    </Box>

                    <Box sx={{ flex: 1, display: { xs: "none", sm: "block" } }}>
                        <StatusPill active={sub.status} />
                    </Box>
